...
<html>
  ...
  <body>
    ...
    <button name="gua" onclick="flag=1;changeFlag()">xjs级</button>
    <button name="gua" onclick="flag=2;changeFlag()">礼貌用语级</button>

    <div>
      <div class="copy-status">
        <span class="success">复制成功</span>
        <span class="flag0">普通战力</span>
        <span class="flag1">xjs级</span>
        <span class="flag2">礼貌用语级</span>
      ...
      <h5>做了数据分级，普通等级common，礼貌用语qiu，骂娘等级yang(数量最多)，不加参数默认是common。调用示例 http://nmsl.shenghuo2.cn/maren.php?catalog=yang
          <br>api支持返回纯文本，json，js，不加参数默认返回纯文本。调用示例 http://nmsl.shenghuo2.cn/maren.php?format=json
          <br>api支持返回utf8,或者gbk2312，不加参数默认为utf8。调用示例 http://nmsl.shenghuo2.cn/maren.php?charset=gbk2312
      </h5>
    </div>
    <script>
      (function () {
